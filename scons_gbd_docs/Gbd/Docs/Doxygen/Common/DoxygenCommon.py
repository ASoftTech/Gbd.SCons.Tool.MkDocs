        ...
        if data.get("GENERATE_" + k, v[0]) == "YES":
            ...
            if k != "MAN":
                ...
                if v[4] and v[4] in data:
                    ...
                    fname = v[2] + v[3]
                of = env.File(path.join(out_dir, data.get(k + "_OUTPUT", v[1]),
                              fname))
                targets.append(of)
                # don't clean single files
                # we remove the complete output folders (see above)
                env.NoClean(of)
            else:
...
def doxygen_file_parse(file_contents, conf_dir, data=None):
        ...
        if token in ['\n']:
            ...
            if token == "+=":
                ...
                if key == "TAGFILES" and key in data:
                    ...
                    # don't reset the @INCLUDE list
                    # when we see a new @INCLUDE line.
                    pass
                else:
                    data[key] = []
            elif key == "@INCLUDE":
    ...
    for (k, v) in data.copy().items():
        ...
        # items in the following list will be kept as lists
        # and not converted to strings
